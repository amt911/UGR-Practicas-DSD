...
\section{Estructura de archivos y cómo compilar}
Esta sección solo sirve a modo de explicación de cómo están estructuradas las carpetas y cómo se compila esta práctica. En el directorio raíz se encuentran 5 carpetas, 3 de ellas correspondientes a los ejemplos, otra denominada ``Parte 2 Donaciones'', destinada al ejercicio del servidor de donaciones, y una última con las implementaciones anteriores de la segunda parte.
...
\begin{itemize}
    ...
    \item \textbf{script\_cliente\_varios.sh: }Sirve para probar que funciona correctamente el algoritmo en anillo. Lanza dos procesos con un número de clientes pasados por parámetro. Además, incluye una versión insegura para ver cómo fallaría. 
...
En el  main del cliente se activa el gestor de seguridad también, obtiene el ``rmiregistry'' que se encuentra en la dirección pasada por consola, y obtiene a partir del registro el stub del objeto remoto para poder lanzar el método anteriormente mencionado.
...
Esta sección solamente sirve para explicar varias implementaciones que he realizado del algoritmo en anillo hasta dar con el definitivo, que es el que se explica en toda la memoria. Esta parte solo está para una explicación, no es necesario que se tenga en cuenta.
