...
\begin{itemize}
    ...
    \item \textbf{datosClientes: }Map de entero y TransaccionesClientes donde se almacenan para cada cliente el historial de donaciones junto con la fecha en la que se ha realizado.
    ...
    \item \textbf{subTotal: }Contiene el subtotal donado por los clientes en esa réplica. Este atributo luego se usa para obtener el total global. 
\end{itemize}

La clase ``TransaccionesClientes'' sirve para almacenar, para un cliente dado, todas las donaciones junto con la fecha en la que se realizan. Además almacena el subtotal donado para ese cliente.
Esta clase se usa en el Map mencionado anteriormente (datosClientes) para poder asociar a un cliente su historial de transacciones, además de hacer más sencilla la obtención de este historial por el propio cliente.
